...
def env_parallel(ego_model, enm_model):
...
def caculate_task(ego_agent, enm_agent):
    ...
    for i in range(count):
        multi_res.append(pool.apply_async(env_parallel,(ego_agent, enm_agent)))
...
def main():
    ...
    for i in range(count):
        for j in range(count):
            ego_model=ego_model_dir+f"/actor_"+str(i*5)+".pt"
            enm_model=ego_model_dir+f"/actor_"+str(j*5)+".pt"
            print(ego_model)
            winrate_list.append([(i*5,j*5),caculate_task(ego_model,enm_model)])
